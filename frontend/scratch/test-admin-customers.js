...
async function run() {
  ...
  await page.screenshot({ path: path.join(brainDir, 'admin_login_filled_customers.png') });
  ...
  // Navigate directly to customers list page
  console.log('Navigating to admin customers page...');
  await page.goto('http://localhost:5500/frontend/pages/admin/customers.html', {
    waitUntil: 'networkidle0'
  });

  // Wait for the table data to load (no spinner / has data rows)
  console.log('Waiting for customers data to load...');
  ...
  // Take screenshot of the customers page
  await page.screenshot({ path: path.join(brainDir, 'admin_customers_redesign.png') });
  console.log('Saved admin customers page screenshot.');
